...
// ── Channel modes (quota vs tracking-only) ───────────────────────────────────

function setChannelMode(guildId, channelId, mode) {
  const cfg = loadConfig(guildId);
  if (!cfg.channel_modes) cfg.channel_modes = {};
  cfg.channel_modes[channelId] = mode;
  saveConfig(guildId, cfg);
}

function getTrackingOnlyChannels(guildId) {
  const modes = loadConfig(guildId)?.channel_modes || {};
  return Object.keys(modes).filter(id => modes[id] === 'tracking-only');
}
