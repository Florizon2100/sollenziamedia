...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    ...
    if (commandName === 'pause')           return await handlePause(interaction);
    if (commandName === 'unpause')         return await handleUnpause(interaction);
    if (commandName === 'pauselist')       return await handlePauseList(interaction);
...
async function handleAnalytics(interaction) {
  ...
  const channelName = (interaction.channel ?? await interaction.guild.channels.fetch(channelId))?.name ?? 'unknown';
...
// ─── /pause /unpause /pauselist ───────────────────────────────────────────────

async function handlePause(interaction) {
  if (!requireAdmin(interaction)) return;
  const channel = interaction.options.getChannel('channel');
  const added   = pauseChannel(interaction.guildId, channel.id);
  await interaction.reply({ content: added ? `⏸️ <#${channel.id}> is now paused — no more alerts for this channel.` : `ℹ️ <#${channel.id}> is already paused.`, ephemeral: true });
}

async function handleUnpause(interaction) {
  if (!requireAdmin(interaction)) return;
  const channel  = interaction.options.getChannel('channel');
  const removed  = unpauseChannel(interaction.guildId, channel.id);
  await interaction.reply({ content: removed ? `▶️ <#${channel.id}> is now unpaused — alerts resumed.` : `❌ <#${channel.id}> was not paused.`, ephemeral: true });
}

async function handlePauseList(interaction) {
  if (!requireAdmin(interaction)) return;
  const paused = getPausedChannels(interaction.guildId);
  await interaction.reply({
    embeds: [new EmbedBuilder()
      .setColor(0xFEE75C).setTitle('⏸️ Paused Channels')
      .setDescription(paused.length ? paused.map(id => `• <#${id}>`).join('\n') : 'No paused channels.')
      .setTimestamp()],
    ephemeral: true,
  });
}
