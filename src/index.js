...
async function handleAnnouncement(interaction) {
  ...
  for (const channelId of allowedIds) {
    try {
      ...
      // Check bot has MentionEveryone permission in this channel
      const botMember = interaction.guild.members.me;
      const perms = ch.permissionsFor(botMember);
      if (!perms?.has('MentionEveryone')) {
        console.error(`[announcement] Missing MentionEveryone in #${ch.name}`);
        failed++;
        continue;
      }
  ...
  await interaction.editReply({
    content: `✅ Announcement sent to **${sent}** channel(s)${failed ? ` (⚠️ ${failed} failed — bot may be missing \`Mention Everyone\` permission in those channels)` : ''}.`
  });
}
