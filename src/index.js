...
const {
  ...
  getWeekKey, saveAnalytics, getAnalytics, isDuplicateLink,
} = require('./database');
...
const { scheduleDailyCheck, scheduleAnalyticsReminder } = require('./cron');
...
client.once(Events.ClientReady, (c) => {
  ...
  try { scheduleDailyPlatformCheck(client); } catch(e) { console.error('[DAILY-CHECK ERROR]', e.message); }
  try { scheduleAnalyticsReminder(client); } catch(e) { console.error('[ANALYTICS-REMINDER ERROR]', e.message); }
...
async function handlePosted(interaction) {
  ...
  if (isDuplicateLink(guildId, videoUrl)) {
    return interaction.reply({ content: '⚠️ **Duplicate link detected!** This video has already been logged. Post cancelled.', ephemeral: true });
  }
...
async function handlePosted2(interaction) {
  ...
  const dupLink = [link1, link2].find(l => isDuplicateLink(guildId, l));
  if (dupLink) {
    return interaction.reply({ content: `⚠️ **Duplicate link detected!** This video has already been logged: ${dupLink}\nPost cancelled.`, ephemeral: true });
  }
  const user    = interaction.user;
  const channel = interaction.channel;
...
async function handleAnalytics(interaction) {
  ...
  if (total === 0) {
    return interaction.reply({
      content: '❌ Could not parse views. Try: `tiktok:1.2M youtube:500k insta:300k` or just `2M`',
      ephemeral: true
    });
  }
...
async function handleWeeklyLeaderboard(interaction) {
  ...
  if (!entries.length) {
    return interaction.reply({ content: `❌ No analytics for ${weekKey} yet. Ask your team to use /analytics first.`, ephemeral: true });
  }
