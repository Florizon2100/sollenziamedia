...
const { getAlertUsers, getAllGuildIds, getLogChannel, getAllowedChannels, getPausedChannels } = require('./server-config');
...
function scheduleDailyCheck(client) {
  ...
  cron.schedule('0 0 * * *', async () => {
    ...
    for (const guildId of guildIds) {
      ...
      // Find users who posted nothing yesterday (excluding paused)
      const allUsers = Object.values(db.known_users).filter(u => !pausedUsers.includes(u.user_id));
      ...
      const incompleteChannels = allowedIds
        .map(id => {
