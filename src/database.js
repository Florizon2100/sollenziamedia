...
function loadDB(guildId, year, month) {
  ...
  // 1. Try old monthly file for the correct month (legacy format)
  ...
  if (fs.existsSync(oldMonthly)) {
    try {
      ...
      // Only serve to main guild — other guilds use their own files
      if (!guildId || guildId === mainGuildId) return db;
      return { posts: [], known_users: {} };
    } catch {}
  }
