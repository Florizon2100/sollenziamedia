...
function loadDB(guildId, year, month) {
  ...
  const prefix = `${cy}-${mm}`;
  const isMainGuild = guildId === process.env.GUILD_ID;

  // The old shared file only ever belongs to the original/main guild's history.
  // Other guilds must NEVER read from it, or their data would mix with the main guild's.
  if (isMainGuild) {
  ...
  // Guild-specific monthly file (used by the main guild once it has no old file left,
  // and always used by every other guild).
  ...
  if (isMainGuild) {
    // Fallback: legacy posts.json filtered by month (main guild only)
...
function saveDB(guildId, data, year, month) {
  ...
  const mm = String(cm).padStart(2,'0');
  const isMainGuild = guildId === process.env.GUILD_ID;
  // Main guild keeps using the old shared-format file (preserves existing history).
  // Every other guild gets its own dedicated file — never mixed with the main guild's data.
  const savePath = isMainGuild
    ? path.join(dataDir, `posts-${cy}-${mm}.json`)
    : getDBPath(guildId, cy, cm);
...
function getKnownUsersGlobal(guildId) {
  ...
  const files = fs.readdirSync(dataDir).filter(f =>
    ...
    (isMainGuild && (f.startsWith('posts-20') || f === 'posts.json'))
  );
