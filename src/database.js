...
function loadDB(guildId, year, month) {
  ...
  // 1. Try old monthly file (legacy - filter by guildId or main guild)
  ...
  if (fs.existsSync(oldMonthly)) {
    try {
      ...
      // Only use this file for the main guild (it has no guildId field)
      if (guildId === mainGuildId) {
        return db;
      }
      // For other guilds, skip this file entirely
    } catch {}
  }
