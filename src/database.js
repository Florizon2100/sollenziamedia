...
function loadDB(guildId, year, month) {
  ...
  const isMainGuild = !guildId || guildId === mainGuildId;

  // 1. Try exact monthly file posts-YYYY-MM.json (legacy format, main guild only)
  const oldMonthly = path.join(dataDir, `posts-${cy}-${mm}.json`);
  if (fs.existsSync(oldMonthly) && isMainGuild) {
    try {
      const db = JSON.parse(fs.readFileSync(oldMonthly, 'utf8'));
      // Filter posts for the correct month (in case old file has mixed months)
      const filtered = { ...db, posts: (db.posts||[]).filter(p => p.date_key?.startsWith(prefix)) };
      if (filtered.posts.length > 0) return filtered;
    } catch {}
  }

  // 1b. For main guild: scan all legacy files for posts of this month
  if (isMainGuild) {
    try {
      const allFiles = fs.readdirSync(dataDir).filter(f => f.match(/^posts-20\d\d-\d\d\.json$/));
      let allPosts = [], knownUsers = {};
      for (const f of allFiles) {
        const db = JSON.parse(fs.readFileSync(path.join(dataDir, f), 'utf8'));
        allPosts.push(...(db.posts||[]).filter(p => p.date_key?.startsWith(prefix)));
        Object.assign(knownUsers, db.known_users||{});
      }
      if (allPosts.length > 0) return { posts: allPosts, known_users: knownUsers };
    } catch {}
  }

  // 2. Try guild-specific monthly file (new format, other guilds)
...
function saveDB(guildId, data, year, month) {
  ...
  const mm = String(cm).padStart(2,'0');
  const mainGuildId = process.env.GUILD_ID;
  // Main guild saves to legacy format, other guilds save to guild-specific file
  const savePath = (!guildId || guildId === mainGuildId)
    ? path.join(dataDir, `posts-${cy}-${mm}.json`)
    : path.join(dataDir, `posts-${guildId}-${cy}-${mm}.json`);
