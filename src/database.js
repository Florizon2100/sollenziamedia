...
function isDuplicateLink(guildId, videoUrl) {
  // Check current month and previous month for duplicates
  const now = new Date();
  for (let m = 0; m <= 1; m++) {
    const d = new Date(now.getFullYear(), now.getMonth() - m, 1);
    const db = loadDB(guildId, d.getFullYear(), d.getMonth() + 1);
    if (db.posts.some(p => p.video_url === videoUrl)) return true;
  }
  return false;
}
